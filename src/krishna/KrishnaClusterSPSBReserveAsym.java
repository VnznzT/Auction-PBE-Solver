...
public class KrishnaClusterSPSBReserveAsym implements AuctionRound<Double, Double>{
		int subgame;
	int maxrounds;
	PBEContext context;
	...
	public KrishnaClusterSPSBReserveAsym(PBEContext context, int subgame, int maxrounds, double[] reservePrices) {
	...
	public double computeUtility(int i, Double v, List<Strategy<Double,Double>> currentStrategies, Belief beliefDistribution, Double[] bids) {
		...
		if(i==winner && maxBid>=reservePrices[subgame]) {
			...
			reward = v - Math.max(secondBid,reservePrices[subgame]);
